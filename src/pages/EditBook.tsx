...
export const EditBook = () => {
  ...
  const onSubmit = async (data: IBook) => {
    try {
      await updateBook({ id: id!, data: { ...data, copies: Number(data.copies) } }).unwrap();
      ...
      <h1 className="text-3xl font-bold mb-4">Edit Book</h1>
       <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-white p-6 rounded-lg shadow-md">
        ...
        <div>
            ...
            <input {...register('title')} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"/>
        </div>
        <div>
            <label className="block text-sm font-medium text-gray-700">Author</label>
            <input {...register('author')} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"/>
        </div>
        <div>
            <label className="block text-sm font-medium text-gray-700">Genre</label>
            <input {...register('genre')} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"/>
        </div>
        <div>
        ...
        <div>
            ...
            <input type="number" {...register('copies', { min: 0 })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"/>
        </div>
